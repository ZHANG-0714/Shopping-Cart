...
export default {
  ...
  methods: {
    ...
    //保存
    preservation(){
       this.$api.saveUser({ gender:this.gender, year:this.year, month:this.month, day:this.day, nickname: this.nickname}).then(res =>{
         console.log(res)
       })
       .catch();
    },
    ...
    confirm(value) {
      ...
      this.year = dayjs(userInfo.year).format("YYYY");
      this.month = dayjs(userInfo.month).format("MM");
      this.day = dayjs(userInfo.day).format("DD");
      this.show = false;
    }
  },
  mounted() {
    this.$api.queryUser().then(res =>{
      this.value = res.userInfo.year+"年"+res.userInfo.month+"月"+res.userInfo.day+"日"
