  ...
  <div>
    ...
    <van-tabs v-model="active">
      ...
      <van-tab title="商品评论" class="goodsTwo">
        <div v-for="(item,index) in goodsTwo" :key="index" class="mode">
          <!--            当不为匿名显示名字-->
            ...
            <div v-for="item1 in item.user" :key="item1.id">
              <div class="c-img">
                ...
                <div class="user">
                  ...
                  <div>
                    <van-rate v-model="item.rate" :size="15" readonly />
                  </div>
                </div>
                <!-- 时间 -->
          ...
          <div v-if="item.anonymous === true">
            <div class="c-img">
              ...
              <!-- 用户名 -->
              <div class="user">
                <div class="nickname">{{item.comment_nickname}}</div>
                <div>
                  <van-rate v-model="item.rate" :size="15" readonly />
                </div>
              </div>
    ...
    >
      ...
      <!-- 立即购买 -->
      <van-button type="primary" block color="#ff4444" class="button" @click="buynow">立即购买</van-button>
    </van-popup>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    //立即购买
    buynow() {
      this.$router.push({name: "Settlement",query: {two:this.ccc}});
      // this.goodsOne.cid = this.goodsOne.id;
      this.goodsOne.count = this.value;
      this.$router.push({name: "Settlement",query: { items: this.goodsOne, zero:this.zero , value:this.value }});

      localStorage.setItem('items',JSON.stringify(this.goodsOne))
      localStorage.setItem('zero',JSON.stringify(this.zero))
  ...
  mounted() {
    ...
    this.$api
      ...
      .then(res => {
        ...
        this.goodsTwo = res.goods.comment; //商品评论
        console.log(this.goodsOne);
      })
      .catch(err => {
        console.log(err);
