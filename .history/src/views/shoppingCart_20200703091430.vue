<template>
  ...
  <div>
    ...
    <div v-else>
      ...
      <div v-else  class="no">
        <div class="commods">
          <img src="../assets/shop.png" alt="" class="shop">
        </div>
         <div class="empty">您的购物车还是空的</div>
         <div class="go" @click="go">去购物</div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    // 去结算
    settlement() {
        this.$router.push({name: "Settlement",query: { itemm:this.aaa}});
      this.list = this.shopping.filter(item => {
        return item.check === true;
      });
      if (this.list.length > 0) {
        this.list.map(item => {
          ...
          //结算传值
          this.$router.push({name: "Settlement",query: { itemm: this.list, zero:this.zero }});
          localStorage.setItem('itemm',JSON.stringify(this.arr))
