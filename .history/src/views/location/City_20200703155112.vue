...
<!-- 城市排列 -->
    <van-index-bar :index-list="msg">
      <div v-for="(item,index) in msg" :key="index">
        ...
        <van-index-anchor :index="item" v-if="datas[item]&&datas[item].length"/>
        <van-cell :title="item1.name" v-for="(item1,index1) in datas[item]" :key="index1" @click="datacity(item1)"/>
      </div>
    </van-index-bar>
   ...
   value(val){
      ...
      if (val.length === 1 && (val >= 'A' && val <= 'Z')) {
        ...
        this.indexList.forEach(key => {
          ...
          datas[key] = itemList.filter(({name}) => name.indexOf(val) !== -1)
        })
        this.datas = datas
      }
    }
