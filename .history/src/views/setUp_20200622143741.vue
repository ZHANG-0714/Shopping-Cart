...
export default {
  ...
  methods: {
    ...
    //保存
    preservation(){
       this.$api.saveUser({ gender:this.userInfo.gender, year:, month:, day:, nickname: this.userInfo.nickname}).then(res =>{
         console.log(res)
       })
       .catch();
    },
    ...
    confirm(value) {
      ...
      this.year = dayjs(userInfo.year).format("YYYY年");
      this.month = dayjs(userInfo.month).format("YYYY年");
      this.day = dayjs(userInfo.day).format("YYYY年");
      this.show = false;
    }
  },
  mounted() {
