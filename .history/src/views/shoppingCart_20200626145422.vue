      ...
      <div class="commodity" v-for="(item,index) in shopping" :key="index">
        ...
        <div class="money">
          ...
          <div class="list">
            ...
            <div>
              <van-stepper v-model="item.count" theme="round" button-size="20" disable-input @change="increase(item)"/>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    //购物车加减商品
    increase(item){
      this.$api.addition({ count:item.count, id:item.id, mallPrice:item.mallPrice })
      .then(res =>{
        console.log(res)
      })
      .catch(err => {
